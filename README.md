...
A c# terminal game. Late Upload. Project completed on: 9/29/20.  Forgive the disgrace that is visual studio light mode

# An introduction to the program and the report:

The entire program is contained within one class. This has greatly reduced complexity in the program as a whole. This report will be ordered in order of use in the program however if a method requires another method that method will be shown first.

![](images/code1.png)

As you can see above the majority of the code is in methods that are called when required. The vast majority of methods overwrite and return an array.
...
can see the rules are relatively thorough so a user should have no issues comprehending what to do at any stage in the game. Especially as the game does give instructions at each stage.
...
As you can see above the first thing this method does is call the previous Get_player_Coordinate method. It asks the user for an X coordinate then calls the method. The same is repeated for y. The console output for this is screenshotted below:
